...
def test_equality(test_entry_json):
    entry1 = Entry(**test_entry_json)
    entry2 = Entry(**test_entry_json)
...
def test_link_with_intruder_child(test_entry_json):
    entry = Entry(**test_entry_json)
    child = Entry('invalid_id', 'Test Child')
...
def test_contains_with_entry(test_entry_json, test_child_entry_json):
    entry = Entry(**test_entry_json)
    child = Entry(**test_child_entry_json)
    invalid = Entry('invalid_id', 'Test Child')
    entry.link(child)
...
def test_dirs(test_entry_json, test_child_entry_json):
    entry = Entry(**test_entry_json)
    child = Entry(**test_child_entry_json)
    entry.link(child)
    assert os.path.join(entry.proper_name, child.proper_name) in child.dirs[0]
